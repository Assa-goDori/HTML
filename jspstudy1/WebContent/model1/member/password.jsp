...
<%--/WebContent/model1/member/password.jsp
	1. 로그아웃상태인 경우
		- '로그인 하세요' 메세지 출력
		- 다른 사용자의 비밀번호 변경 불가.
		- opener 페이지를 loginForm.jsp 페이지 이동 후 현재페이지 닫기.
	2. pass, chgpass 파라미터 값 저장
	3. pass 비밀번호가 DB에 저장된 비밀번호와 틀리면 '비밀번호 오류' 메세지 출력. passwordForm.jsp 페이지 이동
	4. pass 비밀번호가 DB에 저장된 비밀번호와 같으면 비밀번호 수정. opener 페이지를 updateForm.jsp 페이지 이동 후 현재 페이지 닫기.
--%>    
<%
	...
	if(login == null) {	//로그아웃 상태
		...
		if(pass.equals(mem.getPass())) {	//입력된 비밀번호가 DB에 저장된 비밀번호가 같은 경우
			closer = true;
