...
<head>
<meta charset="EUC-KR">
<title>주유비 관리 페이지입니다.</title>
<script type="text/javascript">
	function fuelinsert() {
		var op = "width=500, height=550, left=150,top=150";
		open("insertfuelForm.do", "", op);
	}
	function allshow() {
		sf = document.f;
		sf.y.value = "";
...
<body>
	<form name="f" method="post" action="fuelForm.do?repairtype=2">
		<c:choose>
	...
	<table class="table">
		...
		<c:forEach var="b" items="${list }">
			...
			<fmt:formatDate value="${b.repairdate }" pattern="yyyy-MM-dd" var="date1"/>
				<td><a href="" onclick="window.open('fuelinfo.do?no=${b.no}','','width=600,height=550,left=150,top=150')">${date1 }</a></td>
				<td><fmt:formatNumber value="${b.fuelprice }" pattern="#,###"/>원</td>
				<td>${b.fuel }L</td>
				<td><fmt:formatNumber value="${b.tot }" pattern="#,###"/>원</td>
				<td></td>
			</tr>
			<c:set var="total" value="${total + b.tot }" />
		</c:forEach>
	</table>
	<input type="button" value="항목추가" onclick="fuelinsert()">
	<br>
	<br>
