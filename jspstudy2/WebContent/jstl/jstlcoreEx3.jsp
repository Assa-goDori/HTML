...
<body>
	...
	1부터 100까지의 합 : ${sum }<br>
	1부터 100까지의 짝수의 합 : ${even_sum }<br>
	1부터 100까지의 홀수의 합 : ${odd_sum }<br>
	...
	<h3>forEach 태그를 이용하여 배열 객체의 두번째 요소부터 세번째 요소까지 출력하기</h3>
	<c:forEach var="i" items="${arr }" varStatus="stat" begin="1" end="2">
		arr[${stat.index }] = ${i }<br>
	</c:forEach>
	
	<h3>forEach 태그를 이용하여 배열 객체의 짝수번째 요소까지 출력하기</h3>
	<c:forEach var="i" items="${arr }" varStatus="stat" step="2">
		arr[${stat.index }] = ${i }<br>
	</c:forEach>
	
	<h3>forEach 태그를 이용하여 배열 객체의 홀수번째 요소까지 출력하기</h3>
